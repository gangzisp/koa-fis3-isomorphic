define('pages/org-rank/main', function(require, exports, module) {
  ...
  return  function (it, opt) {
   ...
   for ( var i = 0 ,len = data.rankList.length; i < len; i++){ loop.index = i+1; item = data.rankList[i]; if(loop.index === 1){ loop.first === true;}else{ loop.first = false; }
          ...
          _$out_.push( '"> <h4>No ',  loop.index , '</h4> <div class="ui-avatar-one"><span style="background-image:url(',  item.logo , ')"></span></div> <div class="desc"> <h2>',  item.name , '</h2> <h5><span>动态:',  item.msg , '</span> / <span>关注:',  item.follow , '</span></h5> <div class="ui-sign">昨天<span class="number">',  item.sign , '</span>人签到</div> </div> </li> '); }
          _$out_.push( ''); }
          _$out_.push( '');
        return _$out_.join('');
      }
